...
from student_welfare_backend.core.api.serializers import ClubSerializer, ClubDetailSerializer
    ...
    def post(request):
        ...
        for row in reader:
            ...
            if is_chapter not in ["True", "False"]:
                responses["failure"].append({
                    "row": row,
                    "detail": "Invalid value for is_chapter"
                })
                continue
            if is_technical not in ["True", "False"]:
                responses["failure"].append({
                    "row": row,
                    "detail": "Invalid value for is_technical"
                })
                continue
            if Club.objects.filter(name=name).exists():
                responses["failure"].append({
                    "row": row,
                    "detail": f"Club {name} already exists"
                })
                continue
            ...
            responses["success"].append({
                "row": row,
                "detail": f"Club {name} created successfully"
            })
